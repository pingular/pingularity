...
const (
	...
	keySpeedDirection   = "speed_direction"   // both|down|up|bidir - which directions to run
	keySpeedRetries     = "speed_retries"     // extra attempts per direction on a transient failure
...
type Values struct {
	...
	// SpeedDirection and SpeedRetries are the engine-agnostic test knobs - whichever
	// engine is selected honors them.
	//
	// SpeedDirection is which directions to test: "both" | "down" | "up" | "bidir".
	// "both" runs download then upload; "bidir" runs both at once (--bidir) to expose
	// full-duplex contention - iperf3 only, since Ookla is always sequential (it
	// falls back to "both"); a single direction halves the per-run data and time.
	// SpeedRetries is the extra attempts per direction on a transient failure - chiefly
	// the server still busy from the previous test; 0 = off. It also covers iperf3's UDP
...
func (c *Controller) sealServers(raw string) string {
	...
	for i := range ts {
		...
		if err != nil {
			continue // keep the plaintext rather than lose the password entirely
		}
		ts[i].Password = pw
	}
	b, err := json.Marshal(ts)
...
func New(ctx context.Context, st *store.Store, def Values, opts ...Option) (*Controller, error) {
	...
	if legacy {
		if _, err := c.store.SetSettingsDiff(ctx, map[string]string{
			keyIperfServers: c.sealServers(c.restoreSealed(iperfServersJSON(c.vals.IperfServers))),
		}); err != nil {
...
func (c *Controller) Reload(ctx context.Context) error {
	...
	if legacy {
		if _, err := c.store.SetSettingsDiff(ctx, map[string]string{
			keyIperfServers: c.sealServers(c.restoreSealed(iperfServersJSON(v.IperfServers))),
		}); err != nil {
...
// SpeedDirection and SpeedRetries are engine-agnostic: both Ookla and iperf3
// honor them.
...
func (c *Controller) mutate(ctx context.Context, fn func(v *Values) map[string]string) error {
	...
	if raw, ok := kv[keyIperfServers]; ok {
		kv[keyIperfServers] = c.sealServers(c.restoreSealed(raw))
	}
...
func normalize(v Values) Values {
	...
	if v.SpeedRetention < 0 {
		v.SpeedRetention = 0
	}
	if v.DowntimeRetention < 0 {
		v.DowntimeRetention = 0
	}
...
func sanitizeIperfServers(ts []IperfTarget) []IperfTarget {
	...
	for _, t := range ts {
		...
		if len(pw) > maxAddr { // same 255 cap the UI enforces
			pw = pw[:maxAddr]
		}
		seen[addr] = true
