...
// failSealCrypter seals nothing: Seal always errors. Used to prove that a sealing
// failure aborts the save instead of silently persisting the credential in the
// clear. Unseal passes values through unchanged (never exercised by these tests -
// the store starts empty, so there is nothing to unseal on load).
type failSealCrypter struct{}

func (failSealCrypter) Seal(string) (string, error)     { return "", errors.New("seal boom") }
func (failSealCrypter) Unseal(s string) (string, error) { return s, nil }
...
func TestSealFailureAbortsSave(t *testing.T) {
	...
	base := Values{Latency: 5 * time.Second, Speed: time.Hour, Timeout: 2 * time.Second, DownAfter: 3, UpAfter: 2}
	c, err := New(ctx, st, base, WithCrypter(failSealCrypter{}))
	if err != nil {
		t.Fatalf("New: %v", err)
	}

	const pw = "SUPERSECRET"
	_, err = c.Update(ctx, Patch{
		IperfServer:  pv("10.0.0.5:5201"),
		IperfServers: []IperfTarget{{Addr: "10.0.0.5:5201", Auth: true, Username: "bob", Password: pw}},
	})
	if err == nil {
		t.Fatalf("Update succeeded despite a Seal failure; the save must abort")
	}

	// Nothing may have reached disk in the clear (or at all, for that key).
	raw, err := st.AllSettings(ctx)
	if err != nil {
		t.Fatalf("AllSettings: %v", err)
	}
	if strings.Contains(raw[keyIperfServers], pw) {
		t.Fatalf("password persisted in the clear after a Seal failure: %s", raw[keyIperfServers])
	}
	// The in-memory values must be untouched too (mutate aborts before broadcast).
	if got := c.Snapshot().IperfServers; len(got) != 0 {
		t.Errorf("failed save still mutated live config: %+v", got)
	}
}
