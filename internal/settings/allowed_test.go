...
// short-circuit to true when its schedule is disabled, deny when enabled with no
// active window, and the two features must gate independently.
func TestSpeedAndLatencyAllowed(t *testing.T) {
	...
	if mk(Values{SchedSpeedEnabled: true}).SpeedAllowed(noon) {
		t.Error("speed: enabled with no windows must deny")
	}
	...
	if mk(Values{SchedLatEnabled: true}).LatencyAllowed(noon) {
		t.Error("latency: enabled with no windows must deny")
	}
