...
// running trace instead of starting duplicates.
func (m *Manager) cachedExit(ctx context.Context, ourASN string) *ExitInfo {
	...
	if done := m.tracing; done != nil {
		// A trace is already running - wait for its (fresh by definition) result
	...
	if want != "" {
		if v, ok := resolveIPv4(ctx, want); ok {
			target = v
		} else {
			fellBack = true
