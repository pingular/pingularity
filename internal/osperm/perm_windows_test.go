...
func TestSecureFileDACLWindows(t *testing.T) {
	...
	// The owning account must retain access.
	if sid, err := currentUserSID(); err == nil && !strings.Contains(sddl, sid) {
		t.Errorf("secured file DACL drops the owning account %s: %s", sid, sddl)
	}
}
