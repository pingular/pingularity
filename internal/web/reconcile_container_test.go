...
// containers: the guard's local-only branch hits `case s.InContainer`, which
// only logs a warning and never rejects. Steady-state that is the documented
// stance (a bridged container NATs everyone to the gateway, so loopback can't
// tell a local user from a LAN one), but DURING a restore the box is running
// on whatever the backup said - possibly "no login, network open" - and the
// login password, the container's one real boundary, may be exactly what the
// backup just switched off. For those few seconds fail closed, in a container
// like anywhere else.
func TestContainerRestoreDoesNotServeAnUnprotectedWindow(t *testing.T) {
	...
	if midCode == http.StatusOK {
		t.Errorf("a container served a remote unauthenticated request DURING the restore: the imported "+
			"settings (no login, network open) were live, and the InContainer branch of the guard warns "+
...
// Outside a reconcile the container stance is unchanged: local-only stays a
// warn-only setting there (it is unenforceable behind NAT, and rejecting would
// just lock the dashboard out), so a remote request is still served.
