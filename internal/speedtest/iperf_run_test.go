...
// argv it sees, shrinking the retry/settle waits, and pre-answering the
// --use-pkcs1-padding capability probe - so no test ever execs a real iperf3.
func installFakeIperf(t *testing.T, handler func(args []string) ([]byte, error)) *[][]string {
	t.Helper()
	pkcs1FlagOnce.Do(func() { pkcs1FlagOK = true }) // settle the --help probe without an exec
	origExec := iperfExec
...
// An auth-rejected first attempt flips the RSA padding once, succeeds, and
// caches the working padding per server - so a second Run starts on PKCS#1
// directly instead of re-paying the doomed OAEP attempt.
func TestIperfRunPaddingFallbackAndCache(t *testing.T) {
	calls := installFakeIperf(t, func(args []string) ([]byte, error) {
	...
	iperfPaddingCache.Delete(fakeRunServer) // isolate from earlier runs
	t.Cleanup(func() { iperfPaddingCache.Delete(fakeRunServer) })

	res, err := it.Run(context.Background())
	if err != nil || res.DownloadMbps != 800 {
		t.Fatalf("first Run: err=%v down=%v, want nil / 800 after the padding flip", err, res.DownloadMbps)
	}
	c := *calls
	if len(c) != 2 || argvHas(c[0], "--use-pkcs1-padding") || !argvHas(c[1], "--use-pkcs1-padding") {
		t.Fatalf("first Run argvs = %v, want OAEP then a single PKCS#1 retry", c)
	}
	if v, ok := iperfPaddingCache.Load(fakeRunServer); !ok || v != true {
		t.Fatalf("padding cache = %v/%v, want pkcs1=true latched for %s", v, ok, fakeRunServer)
	}
	...
	c = *calls
	if len(c) != 3 || !argvHas(c[2], "--use-pkcs1-padding") {
		t.Fatalf("second Run should start on the cached PKCS#1 padding in one call, got %v", c)
	}
}
