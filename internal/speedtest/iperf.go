...
type Iperf struct {
	...
	// RSA auth: all four below required, else the run goes unauthenticated.
...
// greys out the engine and the runtime never picks it.
...
var (
	pkcs1FlagOnce sync.Once
	pkcs1FlagOK   bool
)

// iperfHasPKCS1Flag reports whether this host's iperf3 supports --use-pkcs1-padding
// (OAEP-patched builds do; older ones don't, and they already use PKCS#1 so have nothing
// to fall back to). Probed from `iperf3 --help` once and cached.
func iperfHasPKCS1Flag() bool {
	pkcs1FlagOnce.Do(func() {
		ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
		defer cancel()
		out, _ := exec.CommandContext(ctx, "iperf3", "--help").CombinedOutput()
		pkcs1FlagOK = strings.Contains(string(out), "use-pkcs1-padding")
	})
	return pkcs1FlagOK
}

// iperfPaddingCache remembers, per server address, the RSA padding that last
// authenticated - so a server needing the legacy padding skips the doomed first attempt
// on later runs. In-memory, so it re-learns (one cheap failed handshake) after a restart.
var iperfPaddingCache sync.Map // cacheKey(string) -> bool (pkcs1 that worked)

// tryWithPaddingFallback runs op with *auth. On an auth error - and only if the padding
// can flip (canFlip) and isn't already settled this run (*latched) - it flips *auth's
// padding and retries op once. Success latches the working padding into *auth (later
// directions reuse it) and the per-server cache. Only when BOTH paddings are rejected
// as auth failures are the credentials judged genuinely bad (latched, original error
// returned); a non-auth failure on the flipped attempt says nothing about the padding,
// so it comes back unlatched for the retry loop to handle. Non-auth failures never flip.
func tryWithPaddingFallback(auth *iperfAuth, latched *bool, canFlip bool, cacheKey string, op func(iperfAuth) error) error {
	err := op(*auth)
	if err == nil {
		*latched = true
		if auth.on() {
			iperfPaddingCache.Store(cacheKey, auth.pkcs1)
		}
		return nil
	}
	if *latched || !auth.on() || !canFlip || !isAuthErr(err) {
		return err
	}
	alt := *auth
	alt.pkcs1 = !alt.pkcs1
	...
	if err2 == nil {
		*auth = alt
		*latched = true
		iperfPaddingCache.Store(cacheKey, alt.pkcs1)
		return nil
	}
	if !isAuthErr(err2) {
		// The flipped attempt died for another reason (the post-session busy
		// window, a reset). Don't latch - a transient err2 lets withRetry back
		// off and re-run the whole fallback, instead of pinning the wrong
		// padding onto every later direction.
		return err2
	}
	*latched = true // both paddings rejected -> real auth failure; don't thrash later directions
	return err
}
...
func (i *Iperf) Run(ctx context.Context) (Result, error) {
	...
	defer cleanup()
	// Auto RSA-padding: unless the user forced PKCS#1, start from what last worked for
	// this server (cache), else OAEP. If auth is then rejected the transfers below flip
	// padding once and re-learn (see tryWithPaddingFallback).
	cachePort := port
	if cachePort == "" {
		cachePort = iperfDefaultPort // key by effective port so "host" and "host:5201" share an entry
	}
	cacheKey := host + ":" + cachePort
	canFlip := auth.on() && iperfHasPKCS1Flag()
	if auth.on() && !auth.pkcs1 {
		if v, ok := iperfPaddingCache.Load(cacheKey); ok {
			if b, ok := v.(bool); ok {
				auth.pkcs1 = b
			}
		}
	}
	paddingLatched := false
	...
	if dir == "bidir" {
		...
		err := withRetry(ctx, retries, func() error {
			return tryWithPaddingFallback(&auth, &paddingLatched, canFlip, cacheKey, func(a iperfAuth) error {
				stop := startLoadSampler(ctx)
				var e error
				bd, e = runIperfBidir(ctx, host, port, tp, a)
				load = stop()
				return e
			})
		})
		...
		if dir != "up" {
			...
			dnErr = withRetry(ctx, retries, func() error {
				return tryWithPaddingFallback(&auth, &paddingLatched, canFlip, cacheKey, func(a iperfAuth) error {
					...
					var e error
					dn, e = runIperf(ctx, host, port, tp, a, true)
					dnLoad = stop()
					return e
				})
			})
		...
		if dir != "down" {
			...
			upErr = withRetry(ctx, retries, func() error {
				return tryWithPaddingFallback(&auth, &paddingLatched, canFlip, cacheKey, func(a iperfAuth) error {
					stop := startLoadSampler(ctx)
					var e error
					up, e = runIperf(ctx, host, port, tp, a, false)
					upLoad = stop()
					return e
				})
			})
		...
		// surviving error never reaches the caller. Say so, or "upload has been
		// blank for weeks" has no evidence on any surface: the run takes the
		// success path, so speed.fail never increments either.
		if i.Log != nil && dir == "both" {
			if dnErr != nil {
	...
	if iperfUDP(i.UDPFn) {
		...
		udpErr := withRetry(ctx, retries, func() error {
			return tryWithPaddingFallback(&auth, &paddingLatched, canFlip, cacheKey, func(a iperfAuth) error {
				var e error
				loss, jit, e = measureUDP(ctx, host, port, rate, downstream, tp, a)
				return e
			})
		})
		if loss != nil {
...
// an error if the address is malformed or unreachable within ctx. It transfers no data
// and closes immediately - the server logs a stray connection at worst.
...
// and it settles near the bloat. The server accepts then waits for iperf3's cookie; we
// close at once, timing only the handshake (harmless). Returns nil when too few probes
// land (host unreachable) so the caller falls back to min_rtt, then the idle baseline.
...
// removes it. Incomplete credentials yield no auth - the run goes unauthenticated, and an
// auth-requiring server rejects it with a clear error.
