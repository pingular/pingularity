...
reports `1.2.3`. Tags must be `vMAJOR.MINOR.PATCH`; the update feed compares
semver, so a malformed tag will build but never register as "newer".
...
- **GitHub Releases** - the `.tar.gz` archives (`.zip` on Windows), the `.deb`
  and `.rpm` packages, and a `checksums.txt` covering all of them.
- **GHCR** - the multi-arch image `ghcr.io/pingular/pingularity` (amd64 + arm64
  under one tag), tagged with the version and `latest`.
- **Homebrew tap** - a formula commit to `pingular/homebrew-tap` so
  `brew install pingular/tap/pingularity` resolves to the new version.
...
touching users: the GitHub release is marked pre-release, the brew formula and
...
of failing with `422 already_exists`.
...
| `TAP_GITHUB_TOKEN` (PAT) | One PAT with write access to **both** `pingular/homebrew-tap` and `pingular/winget-pkgs` | The single cross-repo secret the pipeline reads (`release.yml` passes it; `.goreleaser.yaml` uses it for the brew formula commit and the winget manifest branch). Store as a repo secret under exactly this name. |
