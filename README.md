...
an `EnvironmentFile` for flags, and runs the daemon as root - required for the
raw-socket traceroute behind the **Exit** panel):
...
- **`--network=host`** (load-bearing) - Pingularity measures *your host's*
  ...
  **Docker Desktop (macOS, Windows) cannot do this** - host networking is a
  Linux feature, and the container runs inside Desktop's Linux VM either way. So
  on those platforms the readings are always the container's view rather than
  your machine's. Everything still works; the numbers just describe a different
  path, and the dashboard says so when it detects it. Run it natively on macOS
  or Windows if you want the measurements to match the host.
...
- **Connection** - public IP (v4·v6), ISP + geolocation, the actual upstream
  ...
  this needs root or `ping_group_range` and is silently omitted otherwise -
...
>
> The full picture - what the trust boundary is, why the service runs as root,
> what the defaults protect and how to deploy it safely - is in
