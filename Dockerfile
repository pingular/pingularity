# Minimal image around the prebuilt static binary. goreleaser's docker build
# passes the compiled `pingularity` for the target arch into the context.
#
...
FROM --platform=$BUILDPLATFORM debian:12-slim AS setcap
RUN apt-get update \
