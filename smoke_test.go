...
func TestBuildAndBoot(t *testing.T) {
	...
	// Free the reserved port the instant before the child binds it. Holding the
	// listener open through all the setup above means nothing else on the host
	// could have grabbed the port meanwhile, so the close-then-reuse window shrinks
	// to this single statement (a fully gap-free handoff would need the child to
	// inherit the listener fd, which the `run` command doesn't accept).
	releasePort()
...
// reserveHighPort reserves a currently-free loopback port in the 19000-19999
// range (never 9000, which a real install owns) and returns it with a release
// func that frees it. Unlike a bind-then-immediately-close probe, the listener is
// held OPEN until release is called, so no other process can take the port in the
// meantime; the caller releases it immediately before starting the child, keeping
// the close-then-reuse window as small as possible. release is idempotent and
// also wired to t.Cleanup, so the port is freed even if the caller never starts
// the child. Skips if none is free.
func reserveHighPort(t *testing.T) (port string, release func()) {
	t.Helper()
	for p := 19000; p < 20000; p++ {
		...
		if err == nil {
			closed := false
			release = func() {
				if !closed { // called once before Start and once via Cleanup; guard the double close
					closed = true
					l.Close()
				}
			}
			t.Cleanup(release)
			return strconv.Itoa(p), release
		}
	}
